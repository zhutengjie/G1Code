...
class LeggedRobot_location(LeggedRobot):
    ...
    def _reset_task(self, env_ids):
        ...
        a = torch.rand(n, device=self.device, dtype=torch.float32) * 4
        b = torch.rand(n, device=self.device, dtype=torch.float32)
        x_noises = a * torch.cos(np.pi * b)
        y_noises = a * torch.sin(np.pi * b)
        rand_pos = torch.cat([x_noises.unsqueeze(1), y_noises.unsqueeze(1), torch.zeros((n, 1), device=self.device, dtype=torch.float32)],dim=1)
